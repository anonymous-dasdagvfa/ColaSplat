...
class GaussianModel:
    ...
    def capture(self, include_feature=False):
        if include_feature:
            assert self._language_feature1 is not None, "没有设置language feature1"
            assert self._language_feature2 is not None, "没有设置language feature2"
            assert self._language_feature3 is not None, "没有设置language feature3"
            return (
    ...
    def training_setup(self, training_args):
        ...
        if training_args.include_feature:
            if self._language_feature1 is None or self._language_feature1.shape[0] != self._xyz.shape[0]:
                language_feature1 = torch.zeros((self._xyz.shape[0], 3), device="cuda")
                ...
                self._language_feature1 = nn.Parameter(language_feature1.requires_grad_(True))
            ...
            l = [
                ...
                {'params': [self._language_feature1], 'lr': training_args.language_feature_lr, "name": "language_feature1"},
            ...
            assert self._language_feature1 is None, "在训练原始gs的时候language feature1应该始终为None"
            assert self._language_feature2 is None, "在训练原始gs的时候language feature2应该始终为None"
            assert self._language_feature3 is None, "在训练原始gs的时候language feature3应该始终为None"
    ...
    def construct_list_of_attributes_quant(self, save_att=None):
        # l = ['x', 'y', 'z', 'nx', 'ny', 'nz']
        l = []
        if 'xyz' in save_att:
            l += ['x', 'y', 'z']
        ...
        if 'rotation' in save_att:
            for i in range(self._rotation.shape[1]):
                l.append('rot_{}'.format(i))
        if 'language_feature1' in save_att:
            for i in range(self._language_feature1.shape[1]):
                l.append('language_feature1_{}'.format(i))
        if 'language_feature2' in save_att:
            for i in range(self._language_feature2.shape[1]):
                l.append('language_feature2_{}'.format(i))
        if 'language_feature3' in save_att:
            for i in range(self._language_feature3.shape[1]):
                l.append('language_feature3_{}'.format(i))
        return l
    ...
    def save_ply(self, path):
        ...
        language_feature1 = self._language_feature1.detach().cpu().numpy()
        ...
        attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, language_feature1, language_feature2, language_feature3), axis=1)
        
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)

    def save_non_quant_ply(self, path, save_attributes=None):
        # save_attributes 控制最终保存进 .ply 文件中的属性列有哪些。
        mkdir_p(os.path.dirname(path))
        ...
        # 存储白名单 只有在save_attributes里才会被存储进ply文件 其他被量化
        if save_attributes is None:
            save_attributes = list(all_attributes.keys())
        
        dtype_full = [(attribute, 'f4') for attribute in self.construct_list_of_attributes_quant(save_attributes)]
    ...
    # 把 PLY 文件里的高斯点云重新读回内存，同时按需“解码”量化过的属性
    def load_ply(self, path, quant=False):
        ...
        if quant:
            ...
            # 检查文件是否都存在
            if not (os.path.exists(inds_file) and os.path.exists(codebook_file) and os.path.exists(args_file)):
                print(f"[Warning] Missing kmeans files in {base_path}. Skipping loading.")
            else:
        ...
        if 'xyz' in quant_params and quant:
            xyz = np.expand_dims(codebook['xyz'][indices_dict['xyz']].cpu().numpy(), -1)
            print("⚠️⚠️⚠️ load xyz from quant codebook")
        else:
        ...
        if 'dc' in quant_params and quant:
            features_dc = np.expand_dims(codebook['dc'][indices_dict['dc']].cpu().numpy(), -1)
            print("⚠️⚠️⚠️ load dc from quant codebook")
        else:
        ...
        if 'sh' in quant_params and quant:
            ...
            print(f"Expected length: {3*(self.max_sh_degree + 1) ** 2 - 3}")
        ...
        if 'scale' in quant_params and quant:
            scales = codebook['scale'][indices_dict['scale']].cpu().numpy()
            print("⚠️⚠️⚠️ load scale from quant codebook")
        else:
        ...
        if 'rot' in quant_params and quant:
            rots = codebook['rot'][indices_dict['rot']].cpu().numpy()
            print("⚠️⚠️⚠️ load rot from quant codebook")
        else:
        ...
        if 'language_feature1' in quant_params and quant:
            language_feature1 = codebook['language_feature1'][indices_dict['language_feature1']].cpu().numpy()
            print("⚠️⚠️⚠️ load language_feature1 from quant codebook")
        else:
        ...
        if 'language_feature2' in quant_params and quant:
            language_feature2 = codebook['language_feature2'][indices_dict['language_feature2']].cpu().numpy()
            print("⚠️⚠️⚠️ load language_feature2 from quant codebook")
        else:
        ...
        if 'language_feature3' in quant_params and quant:
            language_feature3 = codebook['language_feature3'][indices_dict['language_feature3']].cpu().numpy()
            print("⚠️⚠️⚠️ load language_feature3 from quant codebook")
        else:
    ...
    def prune_points(self, mask):
        ...
        self._opacity = optimizable_tensors["opacity"]
        if hasattr(self, "_language_feature1"):
            self._language_feature1 = optimizable_tensors["language_feature1"]
        if hasattr(self, "_language_feature2"):
            self._language_feature1 = optimizable_tensors["language_feature2"]
        if hasattr(self, "_language_feature3"):
            self._language_feature1 = optimizable_tensors["language_feature3"]
    ...
    def prune_points_admm(self, mask):
        ...
        self._opacity = optimizable_tensors.get("opacity", self._opacity[valid_points_mask])
        ...
        self._language_feature1 = optimizable_tensors.get("language_feature", self._language_feature1[valid_points_mask])
        self._language_feature2 = optimizable_tensors.get("language_feature2", self._language_feature2[valid_points_mask])
        self._language_feature3 = optimizable_tensors.get("language_feature3", self._language_feature3[valid_points_mask])
